...
function imbalance2_posted_by() {
	printf( __( '<span class="meta-sep">di </span> %1$s', 'imbalance2' ),
		sprintf( '<a href="%1$s" title="%2$s">%3$s</a>',
			get_author_posts_url( get_the_author_meta( 'ID' ) ),
			sprintf( esc_attr__( 'Leggi tutti gli articoli di %s', 'imbalance2' ), get_the_author() ),
			get_the_author()
		)
	);
}
...
function imbalance2_tags() {
	...
	if ( $tag_list ) printf(__( '<div class="entry-tags"><span>Tags </span><br/><span style="font-size:12px;font-weight:normal"> %1$s</span></div>', 'imbalance2' ), $tag_list );
}
endif;
